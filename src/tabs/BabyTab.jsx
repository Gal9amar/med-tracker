...
export default function BabyTab({ data, update, profileName, profile, babyLog: babyLogProp }) {
  const { offset, dateKey, dateLabel, isToday, goBack, goForward } = useDateNav()

  const allBabyLog = babyLogProp || data.babyLog || []
  const dayLog = allBabyLog.filter(l => l.profileId === data.activeProfile && l.date === dateKey)

  const feeds    = dayLog.filter(l => l.type === 'feed')
  const diapers  = dayLog.filter(l => l.type === 'diaper')
  ...
  const addFeed = () => {
    update(d => ({ ...d, babyLog: [...(d.babyLog || []), { id: uid(), profileId: d.activeProfile, date: dateKey, type: 'feed', ...feedForm }] }))
    setFeedModal(false)
    setFeedForm({ type: 'breast', amount: '', duration: '', time: timeNow(), notes: '' })
  }

  const addDiaper = (kind) => {
    update(d => ({ ...d, babyLog: [...(d.babyLog || []), { id: uid(), profileId: d.activeProfile, date: dateKey, type: 'diaper', kind, time: timeNow() }] }))
  }
  ...
  const toggleVitamin = (name) => {
    ...
    if (exists) {
      ...
      update(d => ({ ...d, babyLog: [...(d.babyLog || []), { id: uid(), profileId: d.activeProfile, date: dateKey, type: 'vitamin', name, time: timeNow() }] }))
    }
  }
  ...
  return (
    <div>
      {/* Baby header */}
      <div style={{ background: 'linear-gradient(135deg,#f472b622,#fb923c22)', border: '1px solid #f472b633', borderRadius: 14, padding: 14, marginBottom: 14 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <span style={{ fontSize: 36 }}>{profile?.avatar || '👶'}</span>
          <div>
            <h2 style={{ fontSize: 18, fontWeight: 800, color: '#f9a8d4' }}>{profileName}</h2>
            {ageMonths !== null && (
              <p style={{ fontSize: 12, color: '#8b949e', marginTop: 1 }}>
      ...
      <Section title="🍼 האכלה" color="#f472b6">
        {isToday && <button onClick={() => setFeedModal(true)} style={addBtn('#f472b6')}>+ רשום האכלה</button>}
        {feeds.length === 0
      ...
      <Section title="🌸 חיתולים" color="#fb923c">
        {isToday && (
          <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
            <button onClick={() => addDiaper('pee')}  style={{ ...addBtn('#60a5fa'), flex: 1, marginBottom: 0 }}>💧 פיפי</button>
            <button onClick={() => addDiaper('poop')} style={{ ...addBtn('#fb923c'), flex: 1, marginBottom: 0 }}>💩 קקי</button>
            <button onClick={() => addDiaper('both')} style={{ ...addBtn('#a78bfa'), flex: 1, marginBottom: 0 }}>שניהם</button>
          </div>
        )}
        {diapers.length === 0
          ...
          : <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {diapers.map(d => (
                <div key={d.id} style={{ background: '#21262d', borderRadius: 8, padding: '6px 10px', display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  {isToday && <button onClick={() => deleteLog(d.id)} style={{ background: 'none', border: 'none', color: '#6b7280', cursor: 'pointer', fontSize: 12 }}>✕</button>}
                </div>
              ))}
            </div>
        }
      ...
      <Section title="☀️ ויטמינים ותוספים" color="#fbbf24">
        {VITAMIN_DEFAULTS.map(v => {
          ...
          return (
            <div key={v.name} style={{ ...logCard(taken ? '#fbbf2433' : '#21262d'), border: `1px solid ${taken ? '#fbbf2466' : '#30363d'}`, opacity: taken ? 0.85 : 1 }}>
              ...
              {isToday
                ...
                : <span style={{ fontSize: 12, color: taken ? '#22c55e' : '#6b7280', fontWeight: 700 }}>{taken ? '✓ ניתן' : '—'}</span>
              }
            </div>
          )
        })}
...
function EmptyState({ text }) {
  return <p style={{ fontSize: 12, color: '#6b7280', textAlign: 'center', padding: '10px 0' }}>{text}</p>
}
...
const addBtn = (color) => ({
  ...
  cursor: 'pointer', marginBottom: 10, display: 'block', width: '100%', textAlign: 'center'
})
const delBtn = () => ({
